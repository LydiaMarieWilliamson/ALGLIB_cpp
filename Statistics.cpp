...
// Skewness and kurtosis
   if (stddev != 0.0) {
      for (i = 0; i < n; i++) {
         ...
         *kurtosis = *kurtosis + sqr(v2, _state);
      }
      *skewness = *skewness / n;
...
// Parallelism was tried if: i1 - i0 >= 4 && rmul3((double)(i1 - i0), (double)nfeatures, logbase2((double)nfeatures, _state), _state) >= smpactivationlevel(_state)
...
// Allocate
   ae_vector_set_length(&t, maxint(n, m, _state), _state);
   ae_matrix_set_length(c, m, m, _state);
...
// Allocate
   ae_vector_set_length(&t, maxint(m1, m2, _state), _state);
...
// Allocate
   ae_vector_set_length(&t, maxint(maxint(m1, m2, _state), n, _state), _state);
   ae_vector_set_length(&sx, m1, _state);
   ae_vector_set_length(&sy, m2, _state);
   ae_matrix_set_length(c, m1, m2, _state);
// Replace data with ranks
...
// Variance (using corrected two-pass algorithm)
   xvar = 0.0;
   for (i = 0; i < n; i++) {
      xvar = xvar + sqr(x->xR[i] - xmean, _state);
   }
   xvar = xvar / (n - 1);
   yvar = 0.0;
   for (i = 0; i < m; i++) {
      yvar = yvar + sqr(y->xR[i] - ymean, _state);
   }
   yvar = yvar / (m - 1);
...
// Statistic
   df1 = n - 1;
   df2 = m - 1;
   stat = minreal(xvar / yvar, yvar / xvar, _state);
...
static double wsr_w25(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 4.000000e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
// Result
   ...
   *bothtails = 2 * minreal(*lefttail, *righttail, _state);
   ae_frame_leave(_state);
}
} // end of namespace alglib_impl
...
static double mannwhitneyu_utbln5n5(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 2.611165e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln5n6(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 2.738613e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln5n7(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 2.841993e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln5n8(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 2.927700e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln5n9(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.000000e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln5n10(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.061862e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln5n11(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.115427e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln5n12(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.162278e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln5n13(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.203616e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln5n14(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.240370e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln5n15(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.250000e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln6n6(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 2.882307e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln6n8(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.098387e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln6n9(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.181981e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln6n10(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.253957e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln6n11(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.316625e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln6n12(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.371709e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln6n13(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.420526e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln6n14(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.450000e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln7n7(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.130495e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln7n9(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.334314e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln7n10(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.415650e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln7n11(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.486817e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln7n12(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.500000e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln8n8(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.360672e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln8n9(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.464102e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln8n10(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.554093e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln8n11(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.600000e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln9n9(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.576237e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln9n10(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.650000e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln11n11(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.700000e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
static double mannwhitneyu_utbln13n13(double s, ae_state *_state) {
   ...
   double result;
   result = 0.0;
   x = minreal(2 * (s - 0.000000e+00) / 3.750000e+00 - 1, 1.0, _state);
   tj = 1.0;
   tj1 = x;
...
// N1=5, N2 = 5, 6, 7, ...
   if (minint(n1, n2, _state) == 5) {
      ...
      if (maxint(n1, n2, _state) > 29) {
         ...
         result = mannwhitneyu_uninterpolate(f0, f1, f2, maxint(n1, n2, _state), _state);
      }
      return result;
   }
// N1=6, N2 = 6, 7, 8, ...
   if (minint(n1, n2, _state) == 6) {
      if (maxint(n1, n2, _state) == 6) {
...
// Result
   ...
   for (i = 0; i < tiecount; i++) {
      tmp = tmp - tiesize.xZ[i] * (sqr((double)tiesize.xZ[i], _state) - 1) / 12;
   }
...
// NECnt - count of x[i] != Median
   ...
   *bothtails = minreal(2 * binomialdistribution(minint(gtcnt, necnt - gtcnt, _state), necnt, 0.5, _state), 1.0, _state);
...
// S
   ...
   if (n + m > 2) {
      for (i = 0; i < n; i++) {
         s = s + sqr(x->xR[i] - xmean, _state);
      }
      for (i = 0; i < m; i++) {
         s = s + sqr(y->xR[i] - ymean, _state);
      }
