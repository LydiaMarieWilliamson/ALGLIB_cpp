...
static void avx2_rcopyvx_xaligned(const ae_int_t n, const double *__restrict x, double *__restrict y) {
   ae_int_t i;
   const ae_int_t vecLen = (n >> 2) << 2;
   const __m256d *__restrict pSrc = (const __m256d *)x;
   for (i = 0; i < vecLen; i += 4) {
...
void avx2_rcopyvx(const ae_int_t n, const double *__restrict x, double *__restrict y) {
   ...
   switch (unal) {
      ...
      case 24:{
         y[0] = x[0];
         const ptrdiff_t nDone = 4 - (unal >> 3);
         avx2_rcopyvx_xaligned(n - nDone, x + nDone, y + nDone);
         return;
      }
   }
}

static void avx2_icopyvx_xaligned(const ae_int_t n, const ae_int_t *__restrict x, ae_int_t *__restrict y) {
   const ae_int_t tail = (n * sizeof(ae_int_t)) & 31;
   const ae_int_t even = (n * sizeof(ae_int_t)) - tail;
   const __m256i *__restrict pSrc = (const __m256i *)x;
   const ae_int_t nVec = even >> 5;
...
void avx2_icopyvx(const ae_int_t n, const ae_int_t *__restrict x, ae_int_t *__restrict y) {
   ...
   if (unal == 0) {
      avx2_icopyvx_xaligned(n, x, y);
      return;
   }
   const ae_int_t offset = 32 - unal;
   memmove(y, x, offset);
   const ae_int_t nDone = offset / sizeof(ae_int_t);
   avx2_icopyvx_xaligned(n - nDone, x + nDone, y + nDone);
}
   ...
   for (i = 0; i < m; i++) {
      ...
      if (nUnroll >= 1) {
         ...
         switch (nVec - at) {
            ...
            case 1:
               u0 = _mm256_add_pd(_mm256_mul_pd(pX[at], pRow[at]), u0);
         }
   ...
   for (i = 0; i < m; i++) {
      ...
      if (nUnroll >= 1) {
         ...
         switch (nVec - at) {
            ...
            case 1:
               u0 = _mm256_add_pd(_mm256_mul_pd(pX[at], ULOAD256PD(pRow[at])), u0);
         }
         sum = _mm256_add_pd(_mm256_add_pd(_mm256_add_pd(u0, u1), _mm256_add_pd(u2, u3)), _mm256_add_pd(_mm256_add_pd(u4, u5), _mm256_add_pd(u6, u7)));
      } else {
         switch (nVec) {
...
void avx2_rgemvx_straight(const ae_int_t m, const ae_int_t n, const double alpha, ae_matrix *__restrict a, const ae_int_t ia, const ae_int_t ja, const double *__restrict x, double *__restrict y) {
   ...
   const ptrdiff_t unal = ((ptrdiff_t)x) & 31;
   if (unal == 0) {
      avx2_rgemvx_straight_xaligned(m, n, alpha, a, ia, ja, x, y);
      return;
   }
...
void avx2_rgemvx_transposed(const ae_int_t m, const ae_int_t n, const double alpha, ae_matrix *__restrict a, const ae_int_t ia, const ae_int_t ja, const double *__restrict x, double *__restrict y) {
   ...
   if (unal == 0) {
      avx2_rgemvx_transposed_yaligned(m, n, alpha, a, ia, ja, x, y);
      return;
   }
   const ptrdiff_t shift = 4 - (unal >> 3);
   for (i = 0; i < n; i++) {
   ...
   avx2_rgemvx_transposed_yaligned(m - shift, n, alpha, a, ia, ja + shift, x, y + shift);
}
...
// Run update
   if (urank == 1) {
      for (k = 0; k < uheight; k++) {
         ...
         _mm256_store_pd(target_storage + targetrow, _mm256_sub_pd(_mm256_load_pd(target_storage + targetrow), _mm256_mul_pd(_mm256_broadcast_sd(update_row), u_0123_0)));
      }
   }
   if (urank == 2) {
      for (k = 0; k < uheight; k++) {
         targetrow = raw2smap[superrowidx[k]] * 4;
         double *update_row = rowstorage + offsu + k * urowstride;
         _mm256_store_pd(target_storage + targetrow, _mm256_sub_pd(_mm256_sub_pd(_mm256_load_pd(target_storage + targetrow), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 1), u_0123_1)), _mm256_mul_pd(_mm256_broadcast_sd(update_row), u_0123_0)));
      }
   }
   if (urank == 3) {
      for (k = 0; k < uheight; k++) {
         ...
         _mm256_store_pd(target_storage + targetrow, _mm256_sub_pd(_mm256_sub_pd(_mm256_sub_pd(_mm256_load_pd(target_storage + targetrow), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 2), u_0123_2)), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 1),
                     u_0123_1)), _mm256_mul_pd(_mm256_broadcast_sd(update_row), u_0123_0)));
      }
   }
   if (urank == 4) {
      for (k = 0; k < uheight; k++) {
         ...
         _mm256_store_pd(target_storage + targetrow, _mm256_sub_pd(_mm256_sub_pd(_mm256_sub_pd(_mm256_sub_pd(_mm256_load_pd(target_storage + targetrow), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 3), u_0123_3)),
                     _mm256_mul_pd(_mm256_broadcast_sd(update_row + 2), u_0123_2)), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 1), u_0123_1)), _mm256_mul_pd(_mm256_broadcast_sd(update_row), u_0123_0)));
      }
   }
   return true;
}
...
//
   ...
   // No row scatter, the most efficient code
      for (k = 0; k < uheight; k++) {
         ...
         target = _mm256_add_pd(_mm256_mul_pd(_mm256_broadcast_sd(rowstorage + offsk), v_w0), target);
