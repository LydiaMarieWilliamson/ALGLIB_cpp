...
double rdotv_avx2(const ae_int_t n, const Real *__restrict x, const Real *__restrict y) {
   ae_int_t i;
...
double rdotv2_avx2(const ae_int_t n, const Real *__restrict x) {
   ae_int_t i;
   const ae_int_t avx2len = n >> 2;
   const ae_int_t fmaLen = (avx2len >> 2) << 2;
   const __m256d *__restrict pX = (const __m256d *)(x);
...
void rcopyv_avx2(ae_int_t n, const Real *__restrict x, Real *__restrict y) {
   ae_int_t i;
   const ae_int_t avx2len = n >> 2;
   const ae_int_t tail = avx2len << 2;
...
void rcopymulv_avx2(const ae_int_t n, const double v, const Real *__restrict x, Real *__restrict y) {
   ae_int_t i;
   const ae_int_t avx2len = n >> 2;
   const ae_int_t tail = avx2len << 2;
   const __m256d *__restrict pSrc = (const __m256d *)(x);
...
void icopyv_avx2(const ae_int_t n, const ae_int_t *__restrict x, ae_int_t *__restrict y) {
...
void bcopyv_avx2(const ae_int_t n, const bool *__restrict x, bool *__restrict y) {
...
void rsetv_avx2(const ae_int_t n, const double v, Real *__restrict x) {
   ae_int_t i;
   const ae_int_t avx2len = n >> 2;
...
void bsetv_avx2(const ae_int_t n, const bool v, bool *__restrict x) {
   const ae_int_t tail = n & 31;
   const ae_int_t even = n - tail;
   __m256i *__restrict pDest = (__m256i *)x;
...
void rmulvx_avx2(const ae_int_t n, const double v, double *__restrict x) {
   ...
   switch (unal) {
      ...
      case 24:{
         x[0] *= v;
         const ptrdiff_t nDone = 4 - (unal >> 3);
         rmulv_avx2(n - nDone, v, x + nDone);
         return;
      }
   }
}

void raddv_avx2(const ae_int_t n, const double alpha, const Real *__restrict y, Real *__restrict x) {
   ae_int_t i;
   const ae_int_t avx2len = n >> 2;
...
void raddvx_avx2(const ae_int_t n, const double alpha, const double *__restrict y, double *__restrict x) {
   const ptrdiff_t unal = ((ptrdiff_t)x) & 31;
   if (n <= 4) {
      ae_int_t i;
      for (i = 0; i < n; i++)
   ...
   switch (unal) {
      case 0:
         raddvx_avx_xaligned(n, alpha, y, x);
         return;
      case 8:
         x[2] += alpha * y[2];
      case 16:
      ...
      case 24:
      {
         x[0] += alpha * y[0];
         const ptrdiff_t nDone = 4 - (unal >> 3);
         raddvx_avx_xaligned(n - nDone, alpha, y + nDone, x + nDone);
         return;
      }
   }
}

void rmergemulv_avx2(ae_int_t n, const Real *__restrict y, Real *__restrict x) {
   ae_int_t i;
   const ae_int_t avx2len = n >> 2;
   const __m256d *__restrict pSrc = (const __m256d *)(y);
   __m256d *__restrict pDest = (__m256d *)x;
...
void rmergemaxv_avx2(ae_int_t n, const Real *__restrict y, Real *__restrict x) {
   ae_int_t i;
   const ae_int_t avx2len = n >> 2;
   const __m256d *__restrict pSrc = (const __m256d *)(y);
   __m256d *__restrict pDest = (__m256d *)x;
...
void rmergeminv_avx2(ae_int_t n, const Real *__restrict y, Real *__restrict x) {
   ae_int_t i;
   const ae_int_t avx2len = n >> 2;
   const __m256d *__restrict pSrc = (const __m256d *)(y);
   __m256d *__restrict pDest = (__m256d *)x;
...
double rmaxv_avx2(ae_int_t n, const Real *__restrict x) {
   ae_int_t i;
   const ae_int_t avx2len = n >> 2;
   const ae_int_t tail = avx2len << 2;
   const __m256d *__restrict pSrc = (const __m256d *)(x);
...
double rmaxabsv_avx2(ae_int_t n, const Real *__restrict x) {
...
static void rcopyvx_avx2_xaligned(const ae_int_t n, const double *__restrict x, double *__restrict y) {
   ae_int_t i;
   const ae_int_t vecLen = (n >> 2) << 2;
   const __m256d *__restrict pSrc = (const __m256d *)x;
   for (i = 0; i < vecLen; i += 4) {
...
void rcopyvx_avx2(const ae_int_t n, const double *__restrict x, double *__restrict y) {
   const ptrdiff_t unal = ((ptrdiff_t)x) & 31;
   if (n <= 4) {
      ae_int_t j;
      for (j = 0; j < n; j++)
   ...
   switch (unal) {
      ...
      case 24:{
         y[0] = x[0];
         const ptrdiff_t nDone = 4 - (unal >> 3);
         rcopyvx_avx2_xaligned(n - nDone, x + nDone, y + nDone);
         return;
      }
   }
}

static void icopyvx_avx2_xaligned(const ae_int_t n, const ae_int_t *__restrict x, ae_int_t *__restrict y) {
   const ae_int_t tail = (n * sizeof(ae_int_t)) & 31;
   const ae_int_t even = (n * sizeof(ae_int_t)) - tail;
   const __m256i *__restrict pSrc = (const __m256i *)x;
   const ae_int_t nVec = even >> 5;
...
void icopyvx_avx2(const ae_int_t n, const ae_int_t *__restrict x, ae_int_t *__restrict y) {
   const ptrdiff_t unal = ((ptrdiff_t)x) & 31;
   if (n <= 8) {
      ae_int_t j;
      for (j = 0; j < n; j++)
   ...
   if (unal == 0) {
      icopyvx_avx2_xaligned(n, x, y);
      return;
   }
   const ae_int_t offset = 32 - unal;
   memmove(y, x, offset);
   const ae_int_t nDone = offset / sizeof(ae_int_t);
   icopyvx_avx2_xaligned(n - nDone, x + nDone, y + nDone);
}

void rgemv_straight_avx2(const ae_int_t m, const ae_int_t n, const double alpha, ae_matrix *__restrict a, const double *__restrict x, double *__restrict y) {
   ae_int_t i;
   ae_int_t j;
   const __m256d *__restrict pX = (const __m256d *)x;
      ...
      if (nUnroll >= 1) {
         ...
         switch (nVec) {
            ...
            case 2:
               ...
               break;
            case 3:
               sum = _mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[0], pRow[0]), _mm256_mul_pd(pX[1], pRow[1])), _mm256_mul_pd(pX[2], pRow[2]));
               break;
            case 4:
               sum = _mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[0], pRow[0]), _mm256_mul_pd(pX[1], pRow[1])), _mm256_add_pd(_mm256_mul_pd(pX[2], pRow[2]), _mm256_mul_pd(pX[3], pRow[3])));
               break;
            case 5:
               sum = _mm256_add_pd(_mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[0], pRow[0]), _mm256_mul_pd(pX[1], pRow[1])), _mm256_add_pd(_mm256_mul_pd(pX[2], pRow[2]), _mm256_mul_pd(pX[3], pRow[3]))), _mm256_mul_pd(pX[4], pRow[4]));
               break;
            case 6:
               sum = _mm256_add_pd(_mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[0], pRow[0]), _mm256_mul_pd(pX[1], pRow[1])), _mm256_add_pd(_mm256_mul_pd(pX[2], pRow[2]), _mm256_mul_pd(pX[3], pRow[3]))), _mm256_add_pd(_mm256_mul_pd(pX[4], pRow[4]), _mm256_mul_pd(pX[5], pRow[5])));
               break;
            case 7:
               sum = _mm256_add_pd(_mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[0], pRow[0]), _mm256_mul_pd(pX[1], pRow[1])), _mm256_add_pd(_mm256_mul_pd(pX[2], pRow[2]), _mm256_mul_pd(pX[3], pRow[3]))), _mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[4], pRow[4]), _mm256_mul_pd(pX[5], pRow[5])), _mm256_mul_pd(pX[6], pRow[6])));
               break;
         }
      }
...
void rgemv_transposed_avx2(const ae_int_t m, const ae_int_t n, const double alpha, ae_matrix *__restrict a, const double *__restrict x, double *__restrict y) {
   ae_int_t i;
   ae_int_t j;
...
void rgemvx_straight_avx2_xaligned(const ae_int_t m, const ae_int_t n, const double alpha, ae_matrix *__restrict a, const ae_int_t ia, const ae_int_t ja, const double *__restrict x, double *__restrict y) {
   ae_int_t i;
   ae_int_t j;
   const __m256d *__restrict pX = (const __m256d *)x;
   const ae_int_t nVec = n >> 2;
      ...
      if (nUnroll >= 1) {
         ...
         switch (nVec) {
            ...
            case 1:
               ...
               break;
            case 2:
               sum = _mm256_add_pd(_mm256_mul_pd(pX[0], ULOAD256PD(pRow[0])), _mm256_mul_pd(pX[1], ULOAD256PD(pRow[1])));
               break;
            case 3:
               sum = _mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[0], ULOAD256PD(pRow[0])), _mm256_mul_pd(pX[1], ULOAD256PD(pRow[1]))), _mm256_mul_pd(pX[2], ULOAD256PD(pRow[2])));
               break;
            case 4:
               sum = _mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[0], ULOAD256PD(pRow[0])), _mm256_mul_pd(pX[1], ULOAD256PD(pRow[1]))), _mm256_add_pd(_mm256_mul_pd(pX[2], ULOAD256PD(pRow[2])), _mm256_mul_pd(pX[3], ULOAD256PD(pRow[3]))));
               break;
            case 5:
               sum = _mm256_add_pd(_mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[0], ULOAD256PD(pRow[0])), _mm256_mul_pd(pX[1], ULOAD256PD(pRow[1]))), _mm256_add_pd(_mm256_mul_pd(pX[2], ULOAD256PD(pRow[2])), _mm256_mul_pd(pX[3], ULOAD256PD(pRow[3])))), _mm256_mul_pd(pX[4], ULOAD256PD(pRow[4])));
               break;
            case 6:
               sum = _mm256_add_pd(_mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[0], ULOAD256PD(pRow[0])), _mm256_mul_pd(pX[1], ULOAD256PD(pRow[1]))), _mm256_add_pd(_mm256_mul_pd(pX[2], ULOAD256PD(pRow[2])), _mm256_mul_pd(pX[3], ULOAD256PD(pRow[3])))), _mm256_add_pd(_mm256_mul_pd(pX[4], ULOAD256PD(pRow[4])), _mm256_mul_pd(pX[5], ULOAD256PD(pRow[5]))));
               break;
            case 7:
               sum = _mm256_add_pd(_mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[0], ULOAD256PD(pRow[0])), _mm256_mul_pd(pX[1], ULOAD256PD(pRow[1]))), _mm256_add_pd(_mm256_mul_pd(pX[2], ULOAD256PD(pRow[2])), _mm256_mul_pd(pX[3], ULOAD256PD(pRow[3])))), _mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(pX[4], ULOAD256PD(pRow[4])), _mm256_mul_pd(pX[5], ULOAD256PD(pRow[5]))), _mm256_mul_pd(pX[6], ULOAD256PD(pRow[6]))));
               break;
         }
      }
      const __m128d t = _mm_add_pd(_mm256_extractf128_pd(sum, 0), _mm256_extractf128_pd(sum, 1));
...
void rgemvx_straight_avx2(const ae_int_t m, const ae_int_t n, const double alpha, ae_matrix *__restrict a, const ae_int_t ia, const ae_int_t ja, const double *__restrict x, double *__restrict y) {
   ae_int_t i;
   ae_int_t j;
   if (n <= 3) {
      for (i = 0; i < m; i++) {
   ...
   const ptrdiff_t unal = ((ptrdiff_t)x) & 31;
   if (unal == 0) {
      rgemvx_straight_avx2_xaligned(m, n, alpha, a, ia, ja, x, y);
      return;
   }
   ...
   rgemvx_straight_avx2_xaligned(m, n - shift, alpha, a, ia, ja + shift, x + shift, y);
}

void rgemvx_transposed_avx2_yaligned(const ae_int_t m, const ae_int_t n, const double alpha, ae_matrix *__restrict a, const ae_int_t ia, const ae_int_t ja, const double *__restrict x, double *__restrict y) {
   ae_int_t i;
   ae_int_t j;
   __m256d *__restrict pY = (__m256d *)y;
   const ae_int_t nVec = m >> 2;
...
void rgemvx_transposed_avx2(const ae_int_t m, const ae_int_t n, const double alpha, ae_matrix *__restrict a, const ae_int_t ia, const ae_int_t ja, const double *__restrict x, double *__restrict y) {
   ae_int_t i;
   ae_int_t j;
   if (m <= 3) {
      for (i = 0; i < n; i++) {
   ...
   if (unal == 0) {
      rgemvx_transposed_avx2_yaligned(m, n, alpha, a, ia, ja, x, y);
      return;
   }
   const ptrdiff_t shift = 4 - (unal >> 3);
   for (i = 0; i < n; i++) {
   ...
   rgemvx_transposed_avx2_yaligned(m - shift, n, alpha, a, ia, ja + shift, x, y + shift);
}
...
void ablasf_daxpby_avx2(ae_int_t n, double alpha, const double *src, double beta, double *dst) {
   ...
   // Well optimized: DST := alpha*SRC + beta*DST
      ...
      n4 = (n >> 2) << 2;
      for (i = 0; i < n4; i += 4) {
         __m256d r = _mm256_add_pd(_mm256_mul_pd(avx_alpha, _mm256_loadu_pd(src + i)),
...
bool spchol_updatekernelabc4_avx2(double *rowstorage, ae_int_t offss, ae_int_t twidth, ae_int_t offsu, ae_int_t uheight, ae_int_t urank, ae_int_t urowstride, ae_int_t uwidth, double *diagd, ae_int_t offsd, ae_int_t *raw2smap, ae_int_t *superrowidx, ae_int_t urbase) {
   ae_int_t k;
   ae_int_t targetrow;
   ae_int_t targetcol;
...
// Run update
   if (urank == 1) {
      for (k = 0; k < uheight; k++) {
         ...
         _mm256_store_pd(target_storage + targetrow, _mm256_sub_pd(_mm256_load_pd(target_storage + targetrow), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 0), u_0123_0)));
      }
   }
   if (urank == 2) {
      for (k = 0; k < uheight; k++) {
         targetrow = raw2smap[superrowidx[k]] * 4;
         double *update_row = rowstorage + offsu + k * urowstride;
         _mm256_store_pd(target_storage + targetrow, _mm256_sub_pd(_mm256_sub_pd(_mm256_load_pd(target_storage + targetrow), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 1), u_0123_1)), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 0), u_0123_0)));
      }
   }
   if (urank == 3) {
      for (k = 0; k < uheight; k++) {
         targetrow = raw2smap[superrowidx[k]] * 4;
         double *update_row = rowstorage + offsu + k * urowstride;
         _mm256_store_pd(target_storage + targetrow, _mm256_sub_pd(_mm256_sub_pd(_mm256_sub_pd(_mm256_load_pd(target_storage + targetrow), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 2), u_0123_2)), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 1),
                     u_0123_1)), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 0), u_0123_0)));
      }
   }
   if (urank == 4) {
      for (k = 0; k < uheight; k++) {
         targetrow = raw2smap[superrowidx[k]] * 4;
         double *update_row = rowstorage + offsu + k * urowstride;
         _mm256_store_pd(target_storage + targetrow, _mm256_sub_pd(_mm256_sub_pd(_mm256_sub_pd(_mm256_sub_pd(_mm256_load_pd(target_storage + targetrow), _mm256_mul_pd(_mm256_broadcast_sd(update_row + 3), u_0123_3)),
...
bool spchol_updatekernel4444_avx2(double *rowstorage, ae_int_t offss, ae_int_t sheight, ae_int_t offsu, ae_int_t uheight, double *diagd, ae_int_t offsd, ae_int_t *raw2smap, ae_int_t *superrowidx, ae_int_t urbase) {
   ae_int_t k;
   ae_int_t targetrow;
   ae_int_t offsk;
