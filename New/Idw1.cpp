...
template<typename T> struct Vector {
   ...
   ~Vector<T>() { FreeVector(); }
   T &operator[](Integer n) const { return _X[n]; }
   T &operator[](Integer n) { return _X[n]; }
...
// Check that the matrix has at least length N and that its first N values are finite.
// AlgLib: Copyright 18.06.2010 by Sergey Bochkanov
   bool IsFiniteVector(const Integer N) const {
      Assert(N >= 0, "Matrix::IsFiniteVector: internal error (N < 0)");
      if (N == 0) return true; else if (_N < N) return false;
   // (#) This is used, instead, from Version 3.15.0 on:
   // double V;
   // for (Integer n = 0; n < N; n++) V = 0.01*V + _X[n];
   // return isfinite(V);
      for (Integer n = 0; n < N; n++) if (!isfinite(_X[n])) return false;
      return true;
   }
// private:
   Integer _N; T *_X;
...
template<typename T> struct Matrix {
   ...
   ~Matrix<T>() { FreeMatrix(); }
   T *operator[](Integer Y) const { return _XY[Y]; }
   T *operator[](Integer Y) { return _XY[Y]; }
...
// Check that the matrix has at least Xs x Ys and its values over [0, Xs) x [0, Ys) are all finite.
// AlgLib: Copyright 18.06.2010 by Sergey Bochkanov
   bool IsFiniteMatrix(const Integer Ys, const Integer Xs) const {
      Assert(Xs >= 0, "Matrix::IsFiniteMatrix: internal error (Xs < 0)");
      Assert(Ys >= 0, "Matrix::IsFiniteMatrix: internal error (Ys < 0)");
      if (Xs == 0 || Ys == 0) return true; else if (_Xs < Xs || _Ys < Ys) return false;
...
// This establishes the splitting point for the original interval [Lo, Hi).
   ...
   while (Lo < Hi) {
      ...
      if (LoV[D] <= S) Lo++; // XY[Lo] is where it belongs. Advance Lo.
         ...
         double *HiV = _XY[Hi]; Integer XYs = 2*_Xs + _Ys;
         for (Integer xy = 0; xy < XYs; xy++) Swap(LoV[xy], HiV[xy]);
         Swap(_Tags[Lo], _Tags[Hi--]);
      }
   }
   if (_XY[Lo][D] <= S) Lo++; else Hi--;
   return Lo;
}
...
// Generate leaf if needed.
   if (Hi - Lo <= HiLeafN) { _Node[NodeN++] = Hi - Lo, _Node[NodeN++] = Lo; return; }
// Select which dimension D to split.
// Create a leaf node if the bounding box has size zero.
   Integer D = 0; double Ds = _Kb._CurHi[0] - _Kb._CurLo[0];
   for (Integer X = 1; X < _Xs; X++) {
      double V = _Kb._CurHi[X] - _Kb._CurLo[X];
      if (V > Ds) Ds = V, D = X;
   }
   if (Ds == 0.0) { _Node[NodeN++] = Hi - Lo, _Node[NodeN++] = Lo; return; }
...
// we zero dimension D of the bounding box and repeat the tree construction.
   double S = _Kb._CurLo[D] + 0.5*Ds;
   CopyV(_Kb._Buf._X, 1, &_XY[Lo][D], _XY._Xs, Hi - Lo);
   Integer N = Hi - Lo;
   Integer LoNs = 0, LoIx = Lo; double LoV = _Kb._Buf[0];
   Integer HiNs = 0, HiIx = Lo; double HiV = _Kb._Buf[0];
   ...
   // If all the points have the same D component LoV == HiV,
   // we zero dimension D of the bounding box and repeat the tree construction.
      double CurLo = _Kb._CurLo[D]; _Kb._CurLo[D] = LoV;
      double CurHi = _Kb._CurHi[D]; _Kb._CurHi[D] = HiV;
      GenerateKd(NodeN, SplitN, Lo, Hi, HiLeafN);
      _Kb._CurLo[D] = CurLo, _Kb._CurHi[D] = CurHi;
      return;
   }
   Integer Mid;
   ...
   // Move split to LoV, place one point to the left bin (move to Lo), others - to the right bin
      S = LoV;
      if (LoIx != Lo) {
         for (Integer xy = 0; xy < 2*_Xs + _Ys; xy++) Swap(_XY[LoIx][xy], _XY[Lo][xy]);
         Swap(_Tags[LoIx], _Tags[Lo]);
      }
      Mid = Lo + 1;
   } else {
   // Move split to HiV, place one point to the right bin (move to Hi - 1), others - to the left bin
      S = HiV;
      if (HiIx != Hi - 1) {
         for (Integer xy = 0; xy < 2*_Xs + _Ys; xy++) Swap(_XY[HiIx][xy], _XY[Hi - 1][xy]);
         Swap(_Tags[HiIx], _Tags[Hi - 1]);
      }
      Mid = Hi - 1;
   }
// Generate the 'split' node.
   _Node[NodeN] = 0, _Node[NodeN + 1] = D, _Node[NodeN + 2] = SplitN;
   _Split[SplitN] = S;
   Integer ExOff = NodeN; NodeN += SplitNodeSizeKd, SplitN++;
// Update CurBox, do recursive calls on the split subtrees, restore CurBox.
   double V;
   _Node[ExOff + 3] = NodeN, V = _Kb._CurHi[D], _Kb._CurHi[D] = S;
   GenerateKd(NodeN, SplitN, Lo, Mid, HiLeafN), _Kb._CurHi[D] = V;
   _Node[ExOff + 4] = NodeN, V = _Kb._CurLo[D], _Kb._CurLo[D] = S;
   GenerateKd(NodeN, SplitN, Mid, Hi, HiLeafN), _Kb._CurLo[D] = V;
...
static void NnQueryKd(const KdTree &Kd, KdTreeReq &Kb, const Integer Off) {
   ...
   if (Kd._Node[Off] > 0) { // Leaf node. Process points.
      Integer LoY = Kd._Node[Off + 1], HiY = LoY + Kd._Node[Off];
      for (Integer y = LoY; y < HiY; y++) {
      // Calculate the distance.
         Integer Xs = Kd._Xs; double Dist = 0.0;
         switch (Kd._Metric) {
            case 0:
               for (Integer x = 0; x < Xs; x++) Dist = Max(Dist, fabs(Kd._XY[y][x] - Kb._X[x]));
            break;
            case 1:
               for (Integer x = 0; x < Xs; x++) Dist += fabs(Kd._XY[y][x] - Kb._X[x]);
            break;
            case 2:
               for (Integer x = 0; x < Xs; x++) Dist += Squ(Kd._XY[y][x] - Kb._X[x]);
            break;
         }
      ...
      // We can NOT process the point if the R-criterion fails to hold, i.e. if InR != 0.0 && Dist > R.
         ...
         // * add the current point without replacement otherwise.
            ...
            else if (Dist < Kb._R[0])
            // New points are added or not, depending on their distance.
            // If added, they replace element at the top of the heap
               if (Kb._InKs == 1) Kb._Ix[0] = y, Kb._R[0] = Dist;
               else TagHeapReTop(Kb._R, Kb._Ix, Kb._InKs, Dist, y);
         }
      }
   } else if (Kd._Node[Off] == 0) { // Simple split.
   // D = the dimension to split, S = the split position.
      Integer D = Kd._Node[Off + 1]; double S = Kd._Split[Kd._Node[Off + 2]];
   // Determine the range [LoSubOff, HiSubOff] of chances for subboxes.
      Integer LoSubOff, HiSubOff; bool BestLeft = Kb._X[D] <= S;
      if (BestLeft)
         HiSubOff = Kd._Node[Off + 3], LoSubOff = Kd._Node[Off + 4];
      else
         LoSubOff = Kd._Node[Off + 3], HiSubOff = Kd._Node[Off + 4];
   // Navigate through subboxes.
      ...
      // Update the bounding box and current distance.
         double Dist, T1, V;
         if (IsLo) {
            Dist = Kb._Dist, T1 = Kb._X[D], V = Kb._CurLo[D];
            ...
            Dist = Kb._Dist, T1 = Kb._X[D], V = Kb._CurHi[D];
            if (T1 >= S) switch (Kd._Metric) {
      ...
      // CurK == InKs means decide to dive or not to dive using point position relative to bounding box.
         if (
            ...
            (Kb._CurK < Kb._InKs || Kb._InKs == 0 || Kb._Dist <= Kb._R[0]*Kb._EstF)
         ) NnQueryKd(Kd, Kb, SubOff);
      // Restore the bounding box and distance.
         if (IsLo) Kb._CurLo[D] = V; else Kb._CurHi[D] = V;
         Kb._Dist = Dist;
      }
   }
}
...
// Calculate the distance from the point to the current bounding box.
   ...
   switch (Kd._Metric) {
      case 0: for (Integer x = 0; x < Kd._Xs; x++) {
         double Vx = X[x], LoV = Kd._Lo[x], HiV = Kd._Hi[x];
         Kb._X[x] = Vx, Kb._CurLo[x] = LoV, Kb._CurHi[x] = HiV;
...
void KdTree::TagKd(const RMatrix XY, const ZVector &Tags, const Integer N, const Integer Xs, const Integer Ys, const Integer Metric) {
   ...
   Assert(XY.IsFiniteMatrix(N, Xs + Ys), "KdTree::TagKd: XY contains infinite or NaN values");
// Initialize.
...
// Allocate and initialize.
   ...
   _Kb.MakeKdBuf(*this);
// Initial fill.
   for (Integer n = 0; n < N; n++) CopyV(_XY[n], XY[n], Xs), CopyV(&_XY[n][Xs], XY[n], Xs + Ys), _Tags[n] = Tags[n];
// Determine the bounding box.
   CopyV(_Lo._X, _XY[0], Xs), CopyV(_Hi._X, _XY[0], Xs);
   for (Integer n = 1; n < N; n++) for (Integer x = 0; x < Xs; x++)
      _Lo[x] = Min(_Lo[x], _XY[n][x]), _Hi[x] = Max(_Hi[x], _XY[n][x]);
...
void KdTree::MakeKd(const RMatrix XY, const Integer N, const Integer Xs, const Integer Ys, const Integer Metric) {
   ...
   Assert(XY.IsFiniteMatrix(N, Xs + Ys), "KdTree::MakeKd: XY contains infinite or NaN values");
   ZVector Tags(N); for (Integer n = 0; n < N; n++) Tags[n] = 0;
...
Integer QueryDistKdBuf(const KdTree &Kd, KdTreeReq &Kb, const RVector &X, const double R, const bool Selfie = true) {
   ...
   Assert(X.IsFiniteVector(Kd._Xs), "QueryDistKdBuf: X contains infinite or NaN values!");
...
Integer QueryDistKd(KdTree &Kd, const RVector &X, const double R, const bool Selfie = true) {
   ...
   Assert(X.IsFiniteVector(Kd._Xs), "QueryDistKd: X contains infinite or NaN values!");
...
Integer QueryDistUKdBuf(const KdTree &Kd, KdTreeReq &Kb, const RVector &X, const double R, const bool Selfie = true) {
   ...
   Assert(X.IsFiniteVector(Kd._Xs), "QueryDistUKdBuf: X contains infinite or NaN values!");
...
void ResultXYsKdBuf(const KdTree &Kd, const KdTreeReq &Kb, RMatrix &XY) {
   ...
   Integer Xs = Kd._Xs, Ys = Kd._Ys; ZVector Ix = Kb._Ix; RMatrix KdXY = Kd._XY;
   XY.GrowMatrix(K, Xs + Ys); for (Integer k = 0; k < K; k++) CopyV(XY[k], &KdXY[Ix[k]][Xs], Xs + Ys);
}
...
void ResultTagsKdBuf(const KdTree &Kd, const KdTreeReq &Kb, ZVector &Tags) {
   Integer K = Kb._CurK; if (K == 0) return;
   ZVector Ix = Kb._Ix, KdTags = Kd._Tags;
   Tags.GrowVector(K); for (Integer k = 0; k < K; k++) Tags[k] = KdTags[Ix[k]];
}
...
// Absolute values are used to handle the cases of negative norms, which KFN requests may produce.
   switch (Kd._Metric) {
      case 0:
         for (Integer k = 0; k < K; k++) R[k] = fabs(KbR[k]);
      break;
      case 1:
         for (Integer k = 0; k < K; k++) R[k] = fabs(KbR[k]);
      break;
      case 2:
         for (Integer k = 0; k < K; k++) R[k] = sqrt(fabs(KbR[k]));
      break;
   }
}
...
void IdwB::IdbSetPoints(const RMatrix &XY, const Integer Ps) {
   ...
   Assert(XY.IsFiniteMatrix(Ps, XYs), "IdwB::IdbSetPoints: XY contains infinite or NaN values!");
   _Ps = Ps;
   _XY.GrowVector(Ps*XYs);
   for (Integer xyp = 0, p = 0; p < Ps; p++) for (Integer xy = 0; xy < XYs; xyp++, xy++) _XY[xyp] = XY[p][xy];
}
...
void IdwB::IdbSetUserTerm(const double V) {
   ...
   _PriorType = 0;
   for (Integer Y = 0; Y < _Ys; Y++) _PriorVal[Y] = V;
}
...
void IdwQ::IdwCalcTs(IdwBuf &Buf, const RVector &X, RVector &Y) const {
   ...
   Assert(X.IsFiniteVector(_Xs), "IdwQ::IdwCalcTs: X contains infinite or NaN values");
// Allocate the output.
   Y.GrowVector(_Ys);
// Quick exit for _Ls == 0 (no data set).
   if (_Ls == 0) {
      for (Integer y = 0; y < _Ys; y++) Y[y] = _GlobP[y];
      return;
   }
   if (_Op == 0) { // Textbook Shepard's method.
      ...
      Integer XYs = _Xs + _Ys;
      double ShepP = _ShepP;
      for (Integer y = 0; y < _Ys; y++) Y[y] = 0.0, Buf._YWb[y] = Eps;
      for (Integer p = 0; p < _Ps; p++) {
      // Compute squared distance.
         double V = 0.0;
         for (Integer x = 0; x < _Xs; x++) V += Squ(_ShepXY[p*XYs + x] - X[x]);
      // Compute weight (with small regularizing addition)
         V = 1.0/(Eps + pow(V, ShepP*0.5));
      // Accumulate
         for (Integer y = 0; y < _Ys; y++) Y[y] += V*_ShepXY[p*XYs + _Xs + y], Buf._YWb[y] += V;
      }
      for (Integer y = 0; y < _Ys; y++) Y[y] = Y[y]/Buf._YWb[y] + _GlobP[y];
      ...
      for (Integer k = 0; k < K; k++) {
         double V = Buf._Rb[k];
         V = Squ((R - V)/(R*V + Eps));
         for (Integer y = 0; y < _Ys; y++) Y[y] += V*Buf._XYb[k][_Xs + y], Buf._YWb[y] += V;
      }
      for (Integer y = 0; y < _Ys; y++) Y[y] = Y[y]/Buf._YWb[y] + _GlobP[y];
   } else if (_Op == 2) { // MSTAB.
      ...
      for (Integer k = 0; k < K; k++) {
         ...
         if (SpeedUp) { // An important special case, fast evaluation possible.
            ...
            for (Integer L = 0; L < _Ls; L++) {
               ...
               if (V0 >= 1.0) break;
               double V = V0*V0; V = (1.0 - V)*(1.0 - V)/(V + Lam);
               for (Integer y = 0; y < _Ys; y++) Buf._YWb[L*_Ys + y] += V*Buf._XYb[k][_Xs + L*_Ys + y];
               Buf._Wb[L] += V, Lam *= LamDecay, V0 *= UDecay;
            }
         }
      }
      if (SpeedUp) // Important special case, finish up the evaluations.
         Buf._YWb[0] = Wf0, Buf._Wb[0] = Ws0,
         Buf._YWb[1] = Wf1, Buf._Wb[1] = Ws1;
      for (Integer y = 0; y < _Ys; y++) Y[y] = _GlobP[y];
      for (Integer L = 0; L < _Ls; L++) for (Integer y = 0; y < _Ys; y++) Y[y] += Buf._YWb[L*_Ys + y]/Buf._Wb[L];
...
double IdwQ::IdwCalc1(const double X) {
   ...
   _Buf._X[0] = X, IdwCalcTs(_Buf, _Buf._X, _Buf._Y);
   return _Buf._Y[0];
}
...
double IdwQ::IdwCalc2(const double X, const double Y) {
   ...
   _Buf._X[0] = X, _Buf._X[1] = Y, IdwCalcTs(_Buf, _Buf._X, _Buf._Y);
   return _Buf._Y[0];
}
...
double IdwQ::IdwCalc3(const double X, const double Y, const double Z) {
   ...
   _Buf._X[0] = X, _Buf._X[1] = Y, _Buf._X[2] = Z, IdwCalcTs(_Buf, _Buf._X, _Buf._Y);
   return _Buf._Y[0];
}
...
void IdwR::IdwErrorMetricsViaCalc(const IdwB &Qb) {
   ...
   for (Integer p = 0; p < Ps; p++) {
      for (Integer x = 0; x < Xs; x++) _Q._Buf._X[x] = Qb._XY[p*(Xs + Ys) + x];
      _Q.IdwCalcTs(_Q._Buf, _Q._Buf._X, _Q._Buf._Y);
      for (Integer y = 0; y < Ys; y++) {
         double V = Qb._XY[p*(Xs + Ys) + Xs + y], dV = fabs(V - _Q._Buf._Y[y]);
         _RmsErr += dV*dV, _AveErr += dV, _MaxErr = Max(_MaxErr, dV);
         Rss += dV*dV, Tss += Squ(V - Qb._TmpMu[y]);
      }
   }
...
// Quick exit for empty dataset.
   if (Qb._Ps == 0) {
      _Q._Xs = Xs, _Q._Ys = Ys;
      _Q._GlobP.ReSizeVector(Ys); for (Integer y = 0; y < Ys; y++) _Q._GlobP[y] = 0.0;
      _Q._Op = 0, _Q._Ls = 0;
...
// *	global mean
   ...
   for (Integer y = 0; y < Ys; y++) Qb._TmpMu[y] = 0.0;
   for (Integer p = 0; p < Ps; p++) for (Integer y = 0; y < Ys; y++) Qb._TmpMu[y] += Qb._XY[p*(Xs + Ys) + Xs + y];
   for (Integer y = 0; y < Ys; y++) Qb._TmpMu[y] /= Ps;
// Compute the global prior.
// NOTE:
// *	For the original Shepard's method it is always the mean value.
   _Q._GlobP.GrowVector(Ys); for (Integer y = 0; y < Ys; y++) _Q._GlobP[y] = Qb._TmpMu[y];
   if (Qb._Op != 0) {
   // The algorithm is set to one of the "advanced" versions with search radius which can handle non-mean prior term.
      if (Qb._PriorType == 0) // User-specified prior.
         for (Integer y = 0; y < Ys; y++) _Q._GlobP[y] = Qb._PriorVal[y];
      else if (Qb._PriorType == 3) // Zero prior.
         for (Integer y = 0; y < Ys; y++) _Q._GlobP[y] = 0.0;
   }
   ...
   // Copy the data set.
      ...
      for (Integer p = 0; p < Ps; p++) {
         for (Integer x = 0; x < Xs; x++) _Q._ShepXY[p*(Xs + Ys) + x] = Qb._XY[p*(Xs + Ys) + x];
         for (Integer y = 0; y < Ys; y++) _Q._ShepXY[p*(Xs + Ys) + Xs + y] = Qb._XY[p*(Xs + Ys) + Xs + y] - _Q._GlobP[y];
      }
      _Q._Ps = Ps;
   ...
   // Build a KD search tree.
      ...
      for (Integer p = 0; p < Ps; p++) {
         for (Integer x = 0; x < Xs; x++) Qb._TmpXY[p][x] = Qb._XY[p*(Xs + Ys) + x];
         for (Integer y = 0; y < Ys; y++) Qb._TmpXY[p][Xs + y] = Qb._XY[p*(Xs + Ys) + Xs + y] - _Q._GlobP[y];
      }
   ...
   // Build a KD search tree, prepare input residuals for the first layer of the model.
      ...
      for (Integer p = 0; p < Ps; p++) {
         for (Integer x = 0; x < Xs; x++) Qb._TmpLs[p][x] = Qb._TmpXY[p][x] = Qb._XY[p*(Xs + Ys) + x];
         Qb._TmpTags[p] = p;
         for (Integer y = 0; y < Ys; y++) Qb._TmpLs[p][Xs + y] = Qb._XY[p*(Xs + Ys) + Xs + y] - _Q._GlobP[y];
      }
   ...
   // Iteratively build layer by layer.
      ...
      // For each point, compute the residual from fitting with the current layer.
         for (Integer p = 0; p < Ps; p++) {
            for (Integer x = 0; x < Xs; x++) Qb._TmpX[x] = Qb._TmpLs[p][x];
            Integer K = QueryDistKd(Qb._TmpKd, Qb._TmpX, CurR, true);
            ResultTagsKd(Qb._TmpKd, Qb._TmpTags);
            ResultDistKd(Qb._TmpKd, Qb._TmpR);
            for (Integer y = 0; y < Ys; y++) Qb._TmpWY[y] = 0.0, Qb._TmpW[y] = IdwW0;
            for (Integer k = 0; k < K; k++) {
               double V = Squ(Qb._TmpR[k]/CurR); V = (1.0 - V)*(1.0 - V)/(V + Lam);
               Integer AdIx = Qb._TmpTags[k];
               for (Integer y = 0; y < Ys; y++) Qb._TmpWY[y] += V*Qb._TmpLs[AdIx][Xs + L*Ys + y], Qb._TmpW[y] += V;
            }
            for (Integer y = 0; y < Ys; y++)
               Qb._TmpLs[p][Xs + (L + 1)*Ys + y] = Qb._TmpLs[p][Xs + L*Ys + y] - Qb._TmpWY[y]/Qb._TmpW[y];
         }
      }
   ...
   // Evaluate the report fields.
      ...
      for (Integer p = 0; p < Ps; p++) for (Integer y = 0; y < Ys; y++) {
         double V = fabs(Qb._TmpLs[p][Xs + Qb._Ls*Ys + y]);
         _RmsErr += V*V, _AveErr += V, _MaxErr = Max(_MaxErr, fabs(V));
         Rss += V*V, Tss += Squ(Qb._XY[p*(Xs + Ys) + Xs + y] - Qb._TmpMu[y]);
      }
      _RmsErr = sqrt(_RmsErr/(Ps*Ys)), _AveErr /= Ps*Ys, _R2 = 1.0 - Rss/(Tss == 0.0? 1.0: Tss);
   // Prepare the internal buffer.
      _Q._Buf.IdwMakeBuf(_Q);
...
// Compute sequence of function values.
   ...
   for (Integer s = 0; s < Steps; s++) {
      ...
      for (Integer x = 0; x < Xs; x++) Xc[x] = X0[x]*T + X1[x]*(1 - T);
      Q.IdwCalcBuf(Xc, Yc); for (Integer y = 0; y < Ys; y++) Yv[s][y] = Yc[y];
   }
// Evaluate all differentiability levels (C0, C1) requested by user.
   ...
   // Compute Lipschitz constant for original and increased steps.
      ...
      for (Integer s = 0; s < Steps - 2; s++) for (Integer y = 0; y < Ys; y++)
         Lc0 = Max(Lc0, fabs(Yv[s][y] - Yv[s + 1][y])), Lc1 = Max(Lc1, fabs(Yv[s][y] - Yv[s + 2][y])/2);
      Ok = Ok && (Lc1 <= 0.0001 || Lc0 <= 1.750*Lc1);
   // Differentiate the function, repeat one more time.
      for (Integer s = 0; s < Steps - 1; s++) for (Integer y = 0; y < Ys; y++) Yv[s][y] = Yv[s + 1][y] - Yv[s][y];
      Steps--;
   }
   return Ok;
}
...
// Try all algorithms
   ...
   // Test empty dataset
      ...
      // Test generic evaluation.
         X.ReSizeVector(Xs); for (Integer x = 0; x < Xs; x++) X[x] = RQ.RandomNormal1();
         Y.ReSizeVector(0), R._Q.IdwCalc(X, Y), Ok = Ok && Y._N == Ys; if (!Ok) return Ok;
         for (Integer y = 0; y < Ys; y++) Ok = Ok && Y[y] == 0.0;
   ...
   // Generate a random data set with distinct points, test interpolation properties (the target function is reproduced almost exactly, the model is continuous).
      ...
      // Generate a data set with distinct points.
         ...
         MuY.ReSizeVector(Ys); for (Integer y = 0; y < Ys; y++) MuY[y] = 0.0;
         for (Integer p = 0; p < Ps; ) {
         // Generate a random point.
            for (Integer x = 0; x < Xs; x++) XY[p][x] = RQ.RandomNormal1();
         ...
         // Repeat point generation if it is too close to some other point.
            ...
            for (Integer p0 = 0; p0 < p; p0++) {
               double V2 = 0.0;
               for (Integer x = 0; x < Xs; x++) V2 = Max(V2, fabs(XY[p][x] - XY[p0][x]));
               V = Min(V, V2);
            }
            if (V >= LoR) { // The point is accepted.
               for (Integer y = 0; y < Ys; y++) XY[p][Xs + y] = RQ.RandomNormal1(), MuY[y] += XY[p][Xs + y]/Ps;
               p++;
            }
         }
      // Build the IDW model.
      ...
      // Test that dataset is actually exactly reproduced.
         for (Integer p = 0; p < Ps; p++) {
         // Test generic evaluation.
            for (Integer x = 0; x < Xs; x++) X[x] = XY[p][x];
            Y.ReSizeVector(0), R._Q.IdwCalc(X, Y), Ok = Ok && Y._N == Ys;
            for (Integer y = 0; y < Ys; y++) Ok = Ok && NearR(Y[y], XY[p][Xs + y], Tiny);
            Integer K = RQ.RandomUniformZ(2*Ys + 1);
            Y.ReSizeVector(K); for (Integer n = 0; n < Y._N; n++) Y[n] = 0.0;
         ...
         // Specialized 1, 2, 3-dimensional cases
            if (Ys == 1) switch (Xs) {
               case 1: Ok = Ok && NearR(R._Q.IdwCalc1(X[0]), XY[p][Xs], Tiny); break;
               case 2: Ok = Ok && NearR(R._Q.IdwCalc2(X[0], X[1]), XY[p][Xs], Tiny); break;
               case 3: Ok = Ok && NearR(R._Q.IdwCalc3(X[0], X[1], X[2]), XY[p][Xs], Tiny); break;
            }
         }
      ...
      //	but we can be sure that the model is continuous along the line connecting the two nearest points.
         for (Integer K = 0; K < 2; K++) {
            ...
            for (Integer x = 0; x < Xs; x++) X[x] = XY[p0][x];
            Integer p1 = -1;
            double V = MaxPosReal;
            for (Integer p = 0; p < Ps; p++) {
               double V2 = 0.0;
               for (Integer x = 0; x < Xs; x++) V2 += Squ(X[x] - XY[p][x]);
               if (V2 < V && V2 > 0.0) {
                  p1 = p;
                  for (Integer x = 0; x < Xs; x++) Xx[x] = XY[p][x];
                  V = V2;
               }
            }
            if (p1 < 0) {
               p1 = RQ.RandomUniformZ(Ps);
               for (Integer x = 0; x < Xs; x++) Xx[x] = XY[p1][x];
            }
      ...
      // Test evaluation at remote points.
         X.ReSizeVector(Xs); for (Integer x = 0; x < Xs; x++) X[x] = 1.0E20*(2*RQ.RandomUniformZ(2) - 1);
         R._Q.IdwCalc(X, Y);
         for (Integer y = 0; y < Ys; y++) Ok = Ok && NearR(Y[y], MuY[y], Tiny);
      }
   // Generate a random data set with NONdistinct points, test approximation properties and error reports.
      ...
      // Generate a data set with non distinct points, each point is repeated; compute reference values of the error metrics.
         ...
         double Rss = 0.0, Tss = 0.0;
         for (Integer y = 0; y < Ys; y++) MuY[y] = 0.0;
         for (Integer p = 0; p < Ps/2; ) {
         // Generate two copies of the same point.
            for (Integer xy = 0; xy < Xs + Ys; xy++) XY[2*p + 1][xy] = XY[2*p][xy] = RQ.RandomNormal1();
         // Test the distance between the newly-generated point and other ones.
         // Repeat point generation if it is too close to some other point.
            double V = MaxPosReal;
            for (Integer p0 = 0; p0 < 2*p; p0++) {
               double V2 = 0.0;
               for (Integer x = 0; x < Xs; x++) V2 = Max(V2, fabs(XY[2*p][x] - XY[p0][x]));
               V = Min(V, V2);
            }
            if (V >= LoR) { // Update MuY.
               for (Integer y = 0; y < Ys; y++) MuY[y] += (XY[2*p][Xs + y] + XY[2*p + 1][Xs + y])/Ps;
            // Perturbation the target value.
               for (Integer y = 0; y < Ys; y++) {
                  double V = pow(2.0, RQ.RandomNormal1());
                  XY[2*p][Xs + y] += V, XY[2*p + 1][Xs + y] -= V;
                  V = fabs(V);
         ...
         for (Integer p = 0; p < Ps; p++) for (Integer y = 0; y < Ys; y++) Tss += Squ(XY[p][Xs + y] - MuY[y]);
      ...
      // *	We do not test all evaluation functions, just IdwCalc().
         for (Integer p = 0; p < Ps/2; p++) {
            for (Integer x = 0; x < Xs; x++) X[x] = XY[2*p][x];
            R._Q.IdwCalc(X, Y); for (Integer y = 0; y < Ys; y++) Ok = Ok && NearR(Y[y], 0.5*(XY[2*p][Xs + y] + XY[2*p + 1][Xs + y]), Tiny);
         }
      // Test continuity properties:
   ...
   // Test correct handling of the prior term.
      ...
      // Zero prior (not tested with the textbook Shepard method).
         if (Op != 0) {
            Qb.IdbSetZeroTerm(); IdwR R(Qb); for (Integer x = 0; x < Xs; x++) X[x] = 1.0E20*(2*RQ.RandomUniformZ(2) - 1);
            R._Q.IdwCalc(X, Y); for (Integer y = 0; y < Ys; y++) Ok = Ok && SmallR(Y[y], Tiny);
         }
      {
      // Mean prior.
         Qb.IdbSetConstTerm(); IdwR R(Qb);
         for (Integer x = 0; x < Xs; x++) X[x] = 1.0E20*(2*RQ.RandomUniformZ(2) - 1);
         R._Q.IdwCalc(X, Y); for (Integer y = 0; y < Ys; y++) Ok = Ok && NearR(Y[y], MuY[y], Tiny);
      }
      // User-specified prior (not tested with the textbook Shepard method).
         if (Op != 0) {
            double V = RQ.RandomNormal1();
            Qb.IdbSetUserTerm(V); IdwR R(Qb); for (Integer x = 0; x < Xs; x++) X[x] = 1.0E20*(2*RQ.RandomUniformZ(2) - 1);
            R._Q.IdwCalc(X, Y); for (Integer y = 0; y < Ys; y++) Ok = Ok && NearR(Y[y], V, Tiny);
         }
      }
   }
   return Ok;
...
// *	derivative at x == 0 must be positive and bigger than 0.1
   RMatrix XY(3, 2);
   for (Integer I = 0; I < 3; I++) XY[I][1] = XY[I][0] = (double)(I - 1);
{
...
// *	we test that the function value does not change significantly along the line.
   ...
   for (Integer n = 0; n < Ps; n++) {
      XY[3*n][0] = (double)n/(double)(Ps - 1), XY[3*n][1] = -0.1, XY[3*n][2] = 0.0;
      XY[3*n + 1][0] = (double)n/(double)(Ps - 1), XY[3*n + 1][1] = 0.0, XY[3*n + 1][2] = 1.0;
      XY[3*n + 2][0] = (double)n/(double)(Ps - 1), XY[3*n + 2][1] = 0.1, XY[3*n + 2][2] = 0.0;
   }
{
...
// Continuity when moving away from the data set.
   XY.ReSizeMatrix(1, 2), XY[0][0] = 0.0, XY[0][1] = 1.0;
   double Rad0 = 1.0;
{
