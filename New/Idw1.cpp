...
// AlgLib: Copyright 18.06.2010 by Sergey Bochkanov
   bool IsFiniteVector(const Integer N) const {
      Assert(N >= 0, "IsFiniteVector: internal error (N < 0)");
...
// AlgLib: Copyright 18.06.2010 by Sergey Bochkanov
   bool IsFiniteMatrix(const Integer Ys, const Integer Xs) const {
      Assert(Xs >= 0, "IsFiniteMatrix: internal error (Xs < 0)");
      Assert(Ys >= 0, "IsFiniteMatrix: internal error (Ys < 0)");
...
void KdTree::TagKd(const RMatrix XY, const ZVector &Tags, const Integer N, const Integer Xs, const Integer Ys, const Integer Metric) {
   ...
   Assert(XY.IsFiniteMatrix(N, Xs + Ys), "TagKd: XY contains infinite or NaN values");
// Initialize.
...
void KdTree::MakeKd(const RMatrix XY, const Integer N, const Integer Xs, const Integer Ys, const Integer Metric) {
   ...
   Assert(XY.IsFiniteMatrix(N, Xs + Ys), "MakeKd: XY contains infinite or NaN values");
...
void IdwB::IdbSetPoints(const RMatrix &XY, const Integer Ps) {
   ...
   Assert(XY.IsFiniteMatrix(Ps, XYs), "IdbSetPoints: XY contains infinite or NaN values!");
   _Ps = Ps;
...
void IdwQ::IdwCalcTs(IdwBuf &Buf, const RVector &X, RVector &Y) const {
   ...
   Assert(X.IsFiniteVector(_Xs), "IdwCalcTs: X contains infinite or NaN values");
// Allocate the output.
   Y.GrowVector(_Ys);
