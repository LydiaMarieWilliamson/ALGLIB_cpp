...
// calculate
   ...
   if (mx != 0.0) {
      for (j = 2; j <= n; j++) {
         ...
         xnorm = xnorm + ae_c_mul(t, ae_c_conj(t)).x;
      }
      xnorm = sqrt(xnorm) * mx;
   }
   alphr = alpha.x;
...
double upperhessenberg1norm(RMatrix a, ae_int_t i1, ae_int_t i2, ae_int_t j1, ae_int_t j2, RVector work) {
   ...
   for (i = i1; i <= i2; i++) {
      for (j = ae_maxint(j1, j1 + i - i1 - 1); j <= j2; j++) {
         work->ptr.p_double[j] = work->ptr.p_double[j] + fabs(a->ptr.pp_double[i][j]);
      }
   }
   result = 0.0;
   for (j = j1; j <= j2; j++) {
